...
// explicit low-level launch environment for Local CLI runs. No API-key
// entries are accepted — Zero Two is subscription-login only, so the CLIs
// authenticate through their own login state. BASE_URL is optional and,
// when omitted, the CLI uses its default endpoint. These values are
// local-only and should not be logged or returned outside this machine.
...
// Zero Two supports exactly these agents; prefs for anything else are
// legacy leftovers and get pruned on read/write.
const SUPPORTED_AGENT_IDS: ReadonlySet<string> = new Set(['claude', 'copilot']);
...
// Subscription-only auth: no API-key env vars are accepted here — the CLIs
// authenticate via their own login state.
...
function normalizeRetiredAgentPrefs(prefs: AppConfigPrefs): AppConfigPrefs {
  ...
  if (typeof next.agentId === 'string' && !SUPPORTED_AGENT_IDS.has(next.agentId)) {
    next = { ...next };
    delete next.agentId;
    changed = true;
  }
  ...
  if (next.agentModels) {
    ...
    for (const agentId of Object.keys(nextAgentModels)) {
      if (SUPPORTED_AGENT_IDS.has(agentId)) continue;
...
function filterAllowedKeys(obj: Record<string, unknown>): AppConfigPrefs {
  ...
  return normalizeRetiredAgentPrefs(result as AppConfigPrefs);
}
...
async function doWrite(
  ...
  const normalizedNextWithoutRetiredAgents = normalizeRetiredAgentPrefs(next as AppConfigPrefs);
