...
// Some adapters reject the synthetic `'default'` model id. Those defs declare
// it by omitting DEFAULT_MODEL_OPTION from `fallbackModels` entirely. When the
// chat run produces a null or 'default' model for one of those adapters,
// prefer the first model from the live list last surfaced to the UI, then
// fall back to the def's first concrete fallback id so the spawn layer always
// has a real model to forward.
...
export function resolveModelForAgent(
  ...
  const liveModels = liveModelOrder.get(liveModelCacheKey(def.id)) ?? [];
