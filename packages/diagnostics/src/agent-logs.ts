...
// CLI session logs can grow into the hundreds of MB, so keep a generous-but-
 ...
 * internal diagnostics (provider errors, retries, auth state) that never reach
 * the daemon's stream.
 *
 * Deliberately excluded: secret-bearing files. We only sweep known *log*
 * directories (`*.log` files) — never auth/config files or session
...
export async function buildAgentCliLogSources(options: AgentCliLogOptions): Promise<LogSource[]> {
  ...
  // Effective CLI home honors the user's app-config override
  // (`CLAUDE_CONFIG_DIR`), falling back to the default.
